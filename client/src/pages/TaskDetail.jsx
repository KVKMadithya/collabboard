...
import { 
  ...
  Paperclip, ShieldAlert, Tag, Loader2, AlertCircle, Clock
} from 'lucide-react';

export default function TaskDetail() {
  const { id } = useParams(); // Grabs the task ID from the URL
  ...
  // MOCK AUTHENTICATED USER (Replace with actual Auth context later)
  const currentUser = { name: 'Kavindu', role: 'admin' };
      ...
      <div className="p-8 w-full max-w-7xl mx-auto h-[calc(100vh-80px)] flex flex-col">
        <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/50 text-red-600 dark:text-red-400 p-6 rounded-2xl flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm opacity-80">{error}. Did you set up the GET /api/tasks/:id route in the backend?</p>
          </div>
        </div>
  ...
  const isAssigned = task.assignees?.some(a => a.name === currentUser.name);
    ...
    <div className="p-8 w-full max-w-7xl mx-auto h-[calc(100vh-80px)] flex flex-col animate-fade-in text-gray-900 dark:text-white overflow-y-auto custom-scrollbar">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div className="flex items-start gap-4">
          <button 
            ...
            className="p-2.5 mt-1 bg-white dark:bg-[#121629] border border-gray-200 dark:border-white/10 rounded-xl text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white shadow-sm transition-colors"
          >
            <ArrowLeft size={20} />
          </button>
          <div>
        ...
        <button className="flex items-center justify-center gap-2 bg-white dark:bg-[#121629] border border-gray-200 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/5 px-5 py-2.5 rounded-xl text-sm font-medium transition-colors shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          {/* Subtasks (Read-only for now, can make interactive later) */}
          <div className="bg-white dark:bg-[#121629] p-6 md:p-8 rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm">
            <h3 className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-2">
              ...
              {task.subtasks?.length > 0 ? (
                task.subtasks.map((st, i) => (
                  <label key={i} className="flex items-center gap-3 bg-gray-50 dark:bg-[#0A0D14] p-4 rounded-xl border border-transparent hover:border-gray-200 dark:hover:border-white/10 cursor-pointer transition-colors">
                    <input type="checkbox" className="accent-[#FF2D88] w-4 h-4 rounded border-gray-300" defaultChecked={st.completed} />
        ...
        <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                {task.assignees?.map((person, i) => (
                  <div key={i} className="flex items-center gap-2 bg-gray-50 dark:bg-[#0A0D14] border border-gray-200 dark:border-white/5 rounded-full px-3 py-1.5" title={person.name}>
                    <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-purple-500 to-[#FF2D88] flex items-center justify-center text-[10px] text-white font-bold shadow-sm">
                      {person.initials || person.name.charAt(0)}
                    </div>
                    <span className="text-xs text-gray-700 dark:text-gray-300 font-medium">{person.name}</span>
                  </div>
          ...
          {/* Action Panel */}
          <div className="bg-white dark:bg-[#121629] border border-gray-200 dark:border-white/10 rounded-2xl p-6 shadow-sm space-y-3">
            <button className="w-full bg-gray-50 dark:bg-[#0A0D14] hover:bg-gray-100 dark:hover:bg-white/5 text-gray-700 dark:text-white py-3 rounded-xl text-sm transition-colors border border-gray-200 dark:border-white/10 flex justify-center items-center gap-2 font-medium shadow-sm">
              <Paperclip size={16} /> View Attachments ({task.attachmentsCount || 0})
            </button>
            
            {/* ROLE-BASED AUTHENTICATION LOGIC */}
            {isAssigned || currentUser.role === 'admin' ? (
              <button 
                onClick={handleMarkAsDone}
                disabled={isUpdating || task.status === 'done'}
                className="w-full bg-[#FF2D88] hover:bg-[#ff2d88]/90 disabled:opacity-50 disabled:hover:scale-100 text-white py-3 rounded-xl text-sm font-bold transition-all shadow-[0_4px_14px_rgba(255,45,136,0.2)] hover:shadow-[0_4px_20px_rgba(255,45,136,0.4)] hover:-translate-y-0.5 flex justify-center items-center gap-2"
              >
              ...
              <div className="relative group">
                <button disabled className="w-full bg-gray-100 dark:bg-white/5 text-gray-400 dark:text-gray-500 py-3 rounded-xl text-sm font-bold border border-gray-200 dark:border-white/5 cursor-not-allowed flex justify-center items-center gap-2">
                  <ShieldAlert size={16} /> Assignee Only Action
                </button>
                <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-56 bg-gray-900 dark:bg-[#0A0D14] text-xs text-white dark:text-gray-300 p-3 rounded-xl text-center opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-xl border border-white/10">
                  You must be assigned to this task (or be an admin) to mark it as complete.
                </div>
              </div>
            )}
          </div>
