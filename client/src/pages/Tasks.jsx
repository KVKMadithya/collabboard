...
import { 
  ...
  Paperclip, Users, AlertCircle, Loader2,
  ArrowUpDown, User, Check
} from 'lucide-react';
...
export default function Tasks() {
  ...
  const columns = [
    { id: 'todo', title: 'To do', color: '#A855F7', border: 'border-purple-500', bg: 'bg-purple-500/10', text: 'text-purple-500' },
    { id: 'in-progress', title: 'In progress', color: '#3B82F6', border: 'border-blue-500', bg: 'bg-blue-500/10', text: 'text-blue-500' },
    { id: 'in-review', title: 'In review', color: '#EAB308', border: 'border-yellow-500', bg: 'bg-yellow-500/10', text: 'text-yellow-500' },
    { id: 'done', title: 'Done', color: '#22C55E', border: 'border-green-500', bg: 'bg-green-500/10', text: 'text-green-500' }
  ];
  ...
  return (
    // FIX 1: Removed 'p-8'. App.jsx already pads the screen. Double padding was crushing the board.
    <div className="w-full flex flex-col animate-fade-in text-gray-900 dark:text-white" onClick={() => setActiveDropdown(null)}>
      
      {/* FIX 2: Custom Glassmorphic Scrollbar Injection (Kills the ugly grey bar) */}
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-6 flex-shrink-0">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            {processedTasks.length} tasks • {columns.length} columns • Live sync active
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex bg-gray-100 dark:bg-[#0A0D14]/80 backdrop-blur-xl p-1 rounded-xl border border-gray-200 dark:border-white/10 shadow-[inset_0_2px_10px_rgba(255,255,255,0.02)]">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white"
            >
          ...
          {/* Sort Dropdown */}
          <div className="relative">
            <button onClick={(e) => { e.stopPropagation(); setActiveDropdown(activeDropdown === 'sort' ? null : 'sort'); }} className={`flex items-center gap-2 bg-white dark:bg-[#121629] border hover:bg-gray-50 dark:hover:bg-white/5 px-4 py-2.5 rounded-xl text-sm font-medium transition-all ${activeDropdown === 'sort' || sortBy !== 'newest' ? 'border-[#FF2D88] text-[#FF2D88] dark:text-white' : 'border-gray-200 dark:border-white/10 text-gray-600 dark:text-gray-300'}`}>
          ...
          <button 
            ...
            className="bg-gradient-to-r from-[#FF2D88] to-[#D91E6D] hover:opacity-90 text-white px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-all shadow-[0_0_20px_rgba(255,45,136,0.3)] hover:scale-105 ml-2"
          >
      ...
      {/* FIX 3: THE HOLY GRAIL HEIGHT CALCULATION */}
      {/* By using style={{ height: 'calc(100vh - 240px)' }}, we FORCE the board to stretch to the bottom of the screen, completely overriding the infinity bug in App.jsx! */}
      <div 
        ...
        {columns.map((col) => {
          ...
          return (
            <div key={col.id} className="min-w-[320px] w-[320px] h-full flex flex-col bg-gray-50 dark:bg-[#0A0D14] rounded-2xl border border-gray-200 dark:border-white/5 shadow-sm overflow-hidden">
              ...
              {/* COLUMN HEADER */}
              <div className={`p-4 border-t-4 ${col.border} flex items-center justify-between bg-white dark:bg-[#121629] flex-shrink-0`}>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                {colTasks.length === 0 ? (
                  <p className="text-sm text-gray-400 dark:text-gray-600 text-center mt-6">No tasks yet.</p>
                ) : (
                  colTasks.map((task) => (
                    <div 
                      key={task._id} 
                      onClick={() => navigate(`/tasks/${task._id}`)} 
                      className={`bg-white dark:bg-[#121629] border border-gray-200 dark:border-white/10 border-l-4 ${col.border} rounded-xl p-4 cursor-pointer hover:border-gray-300 dark:hover:border-white/30 hover:-translate-y-1 transition-all duration-200 group shadow-sm dark:shadow-lg dark:shadow-black/20 flex-shrink-0`}
                    >
                      <div className="flex justify-between items-start mb-2">
                        <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded ${task.priority === 'High' ? 'bg-red-50 dark:bg-red-500/20 text-red-600 dark:text-red-400' : task.priority === 'Medium' ? 'bg-orange-50 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400' : 'bg-green-50 dark:bg-green-500/20 text-green-600 dark:text-green-400'}`}>
                          {task.priority || 'Normal'}
                        </span>
                        <button className="text-gray-400 dark:text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity"><MoreVertical size={16}/></button>
                      </div>
                      <h4 className="font-bold text-sm mb-2 group-hover:text-[#FF2D88] transition-colors">{task.title}</h4>
                      <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-2 mb-4">{task.description}</p>
                      
                      {task.tags && task.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mb-4">
                          {task.tags.map(tag => (
                            <span key={tag} className="bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-600 dark:text-gray-300 text-[10px] px-2 py-0.5 rounded">
                              {tag}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="flex items-center justify-between text-gray-400 dark:text-gray-500 text-xs pt-4 border-t border-gray-100 dark:border-white/5">
                        <div className="flex items-center gap-3">
                          <span className="flex items-center gap-1"><MessageSquare size={12}/> {task.commentsCount || 0}</span>
                          <span className="flex items-center gap-1"><Paperclip size={12}/> {task.attachmentsCount || 0}</span>
                        </div>
                        <div className="flex items-center gap-3">
                          {task.assignees?.length > 0 && (
                            <div className="flex -space-x-1.5 mr-1">
                              {task.assignees.map((a, i) => (
                                <div key={i} className="w-5 h-5 rounded-full bg-gradient-to-tr from-purple-500 to-[#FF2D88] flex items-center justify-center text-[8px] text-white border border-white dark:border-[#121629]" title={a.name}>
              ...
              <div className="mt-auto p-3 border-t border-gray-200 dark:border-white/5 bg-white dark:bg-[#121629] flex-shrink-0">
                <button 
                  ...
                  className="w-full py-2 flex items-center justify-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-white/5 rounded-lg transition-colors border border-transparent dark:hover:border-white/10"
                >
